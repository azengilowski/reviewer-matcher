...
# TODO MAKE SURE TO EXCLUDE CONFLICTS
# TODO FILTER OUT GRAD Students
# TRY OTHER PREFERENCE


# https://github.com/garrrychan/matching/blob/master/src/sim_pipeline.py

sys.setrecursionlimit(10000)


RAW_STUDENT_IDS_CSV = '20250812_ReviewersStudent.csv'
RAW_NON_STUDENT_IDS_CSV = '20250812_ReviewersNonStudent.csv'

PAPER_CSV = '20250812_papers_FINAL.csv'
PAPER_CSV_DESCRIPTOR_DELIMITER = ','
REVIEWER_CSV = '20250812_ReviewersAll.csv'
REVIEWER_CSV_DELIMITER = ';'
REVIEWER_NAMES = '20250812_ReviewersAllNameID.csv'
...
with open(RAW_STUDENT_IDS_CSV, newline='\n') as raw_student_id_csv:
    student_id_reader = csv.reader(raw_student_id_csv, delimiter=',', quotechar='"')
    for row in student_id_reader:
        raw_student_ids.append(row[0])


with open(RAW_NON_STUDENT_IDS_CSV, newline='\n') as non_student_id_csv:
    non_student_id_reader = csv.reader(non_student_id_csv, delimiter=',', quotechar='"')
    for row in non_student_id_reader:
        non_student_ids.append(row[0])
...
reviewer_ids = []
reviewer_embeddings = []
papers_to_top_reviewers = []
name_map = {}
paper_author_map = {}

def create_cleaned_row_string(row):
    raw_string = ''
    for i in range(1,len(row)):
        row_string = re.sub(r'[^a-zA-Z]', ' ', row[i])
        raw_string += ' ' + row_string
    raw_string = re.sub(r'\s+', ' ', raw_string).strip()
    return [row[0], raw_string.lower()]

def ranked_preferences(set1, set2):
    pref_scores = {}
    for set1_p in set1:
        set2_prefs = []
        for set2_p in set2:
            similarity = cosine_similarity(set1_p[1].reshape(1, -1), set2_p[1].reshape(1, -1))
            set2_prefs.append([set2_p[0], similarity])
        sorted_list = sorted(set2_prefs, key=lambda x: x[1], reverse=True)
        pref_scores[set1_p[0]] = list(map(lambda x: x[0], sorted_list))
    return pref_scores

with open(PAPER_CSV, newline='\n') as papercsv:
    paperreader = csv.reader(papercsv, delimiter=',', quotechar='"')
    for row in paperreader:
        paper_ids.append(row[0])
        cleaned_papers.append(create_cleaned_row_string(row))
        paper_author_map[row[0]] = row[3]

with open(REVIEWER_CSV, newline='\n') as reviewerscsv:
    reviewerreader = csv.reader(reviewerscsv, delimiter=',', quotechar='"')
    for row in reviewerreader:
        if (row[0] in raw_student_ids):
            reviewer_ids.append(row[0])
            cleaned_reviewers.append(create_cleaned_row_string(row))


with open(REVIEWER_NAMES, newline='\n') as reviewernamescsv:
    namesreader = csv.reader(reviewernamescsv, delimiter=',', quotechar='"')
    for row in namesreader:
        name_map[row[1]] = row[0]

for paper in cleaned_papers:
    paper_embeddings.append([paper[0], model.encode(paper[1])])

for reviewer in cleaned_reviewers:
    reviewer_embeddings.append([reviewer[0], model.encode(reviewer[1])])


reviewer_prefs = []
paper_prefs = []

paper_preferences = ranked_preferences(paper_embeddings, reviewer_embeddings)
for paper_id in paper_preferences.keys():
    filtered_preferences = filter(lambda x: x != paper_author_map[paper_id], paper_preferences[paper_id])
    index_preferences = list(map(lambda x: int(reviewer_ids.index(x)), filtered_preferences))
    paper_preferences[paper_id] = index_preferences

reviewer_preferences = ranked_preferences(reviewer_embeddings, paper_embeddings)
for reviewer_id in reviewer_preferences.keys():
    index_preferences = list(map(lambda x: int(paper_ids.index(x)), reviewer_preferences[reviewer_id]))
    reviewer_preferences[reviewer_id] = index_preferences
...
for paper_id in paper_ids:
    paper_suited.append(Suited(len(paper_suited), paper_preferences[paper_id], 2))

reviewer_suitors = []
for reviewer_id in reviewer_ids:
    reviewer_suitors.append(Suitor(len(reviewer_suitors), reviewer_preferences[reviewer_id], 6))
    ...
    while unassigned:
        ...
        for suitor in unassigned:
            ...
            if (len(suitor.held) < suitor.capacity) and (suitor.proposals < len(suitor.prefList)):
                # print(suitor.id)
                # print(suitor.preference())
                # print(suiteds[suitor.preference()].prefList)
...
def pipeline(run_report=True, matchFile=None):
    '''
    Run the pipeline to create a Matching and
    return the matching csv and associated reports
    '''
    suiteds = paper_suited
    suitors = reviewer_suitors
    ...
    if run_report:
        ...
        df = pd.DataFrame({k: pd.Series([name_map[reviewer_ids[x.id]] for x in v])
                           for k, v in matches.items()})
        df.columns = [f'Paper {paper_ids[suited.id]}' for suited in suiteds]
...
if __name__ == "__main__":
    result_df = pipeline(run_report=False, matchFile=None)
    result_df.to_csv("./20250812results_students_testing_2_cap6.csv", index=False)
